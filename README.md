...
The **explore-server** is closely linked to [gridexplore-app](https://github.com/gridsuite/gridexplore-app). It is the main entry point requested by the grid explore frontend: most user actions in gridexplore-app go through explore-server, which dispatches and orchestrates the corresponding requests to the specific downstream microservices.
